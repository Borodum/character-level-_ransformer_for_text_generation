...
def mirostat_decode(model, start_text, char_to_idx, idx_to_char, device,
                    tau=5.0, eta=0.1, max_length=500, max_context=256):
    """
    tau: target entropy (controls perplexity)
    ...
    for _ in range(max_length):
        with torch.no_grad():
            ...
            # compute surprise = -log(p)
            surprise = -log_probs

            # sort by probability
            sorted_probs, sorted_indices = torch.sort(probs, descending=True)
            sorted_surprise = -torch.log(sorted_probs + 1e-9)
            ...
            truncated_probs = sorted_probs[:cutoff]
            truncated_indices = sorted_indices[:cutoff]

            truncated_probs = truncated_probs / truncated_probs.sum()

            sampled_idx = torch.multinomial(truncated_probs, 1).item()
            next_token = truncated_indices[sampled_idx].item()

            # observed surprise
            observed_surprise = -math.log(truncated_probs[sampled_idx].item() + 1e-9)
...
def main():
    ...
    parser.add_argument("--tau-values", type=float, nargs="+", default=[3.0, 5.0, 7.0])
    parser.add_argument("--eta", type=float, default=0.1)
    parser.add_argument("--runs", type=int, default=5)
    parser.add_argument("--max-length", type=int, default=500)
    parser.add_argument("--report", default="mirostat_report.json")
    ...
    for tau in args.tau_values:
        ...
        for _ in range(args.runs):
            text = mirostat_decode(model, args.prompt, char_to_idx, idx_to_char,
                                   device, tau=tau, eta=args.eta)
